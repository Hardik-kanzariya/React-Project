...
const Home = () => {
  ...
  // Total pages after deletion
  const totalPagesAfterDelete = Math.ceil(filterData.length / productsPerPage);

  // Adjust current page if needed
  if (currentPage > totalPagesAfterDelete) {
    setCurrentPage(totalPagesAfterDelete || 1); // Avoid 0 page, fallback to 1
  }
};
  ...
  const handleSort = (field, type) => {
    ...
    const sorted = [...products].sort((a, b) => {
      ...
      if (type === "asc") return aVal.localeCompare(bVal);
  ...
  return (
    <>
      <h1 className="mt-5 mb-5 text-center">Products Details</h1>
      ...
      <Container>
        <div className="d-flex flex-wrap align-items-center gap-3 mb-4">
          ...
          <Button variant="success" onClick={() => handleSort(sortField, "asc")}>Sort A-Z</Button>
          <Button variant="warning" onClick={() => handleSort(sortField, "desc")}>Sort Z-A</Button>
         <Button variant="secondary" onClick={handleClearSort}>Clear Sort</Button>
